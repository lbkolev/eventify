...
/// struct for typed errors of method [`get_logs_count`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetLogsCountError {
    Status500(),
...
pub async fn get_logs_count(
...
) -> Result<(), Error<GetLogsCountError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v1/logs/count", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetLogsCountError> =

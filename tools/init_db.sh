...
# Check if a custom database name has been set, otherwise default to 'chainthru'
DB_NAME="${POSTGRES_DB:=chainthru}"
# Check if a custom port has been set, otherwise default to '5432'
DB_PORT="${POSTGRES_PORT:=5432}"
# Check if a custom host has been set, otherwise default to 'localhost'
DB_HOST="${POSTGRES_HOST:=localhost}"

FUNCTION_SIGNATURES_CSV="$(dirname "$0")/../migrations/data/function_signatures.csv"
...
sqlx migrate run --source="$(dirname "$0")/../migrations/rdms/postgres"
